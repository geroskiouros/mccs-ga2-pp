...
def _extract_result_ppci_to_pp(net, ppc, ppci):
    ...
    # merged_bus_idx = np.where(merged_bus == True)
    # for i in range(len(merged_bus_idx[0])):
    #     net.res_bus_est.loc[merged_bus_idx[0][i], "p_mw"] = 0
    #     net.res_bus_est.loc[merged_bus_idx[0][i], "q_mvar"] = 0

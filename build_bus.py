...
def ds_union(ar, bus1, bus2, bus_is_pv, bus_is_active, merged_bus):  # pragma: no cover
    ...
    if root1 == root2:
        return
    if bus_is_active[root2] and ~bus_is_pv[root1]:
        ar[root1] = root2
...
@jit(nopython=True, cache=False)
def ds_create(ar, switch_bus, switch_elm, switch_et_bus, switch_closed, switch_z_ohm, bus_is_pv,
              bus_is_active, bus_in_service, merged_bus):  # pragma: no cover
...
def create_bus_lookup_numba(net, bus_index, bus_is_idx):
    ...
    ds_create(ar, switch_bus, switch_elm, switch_et_bus, switch_closed, switch_z_ohm, bus_is_pv, bus_is_active,
              bus_in_service, merged_bus)
...
def create_bus_lookup_numpy(net, bus_index, closed_bb_switch_mask):
    ...
    if net._fused_bb_switches.any():
        ...
        pv_bus = [net["ext_grid"]["bus"].values[eg_is_mask], net["gen"]["bus"].values[gen_is_mask]]
        pv_ref = np.unique(np.hstack(pv_bus))
        ...
        if any(i in fbus or i in tbus for i in pv_ref):
            ...
            for dj in disjoint_sets:
                ...
                if nr_pv_bus > 0:
                    ...
                    if nr_active_bus > 0:
                        # another active bus. Get bus from active_buses_in_set
